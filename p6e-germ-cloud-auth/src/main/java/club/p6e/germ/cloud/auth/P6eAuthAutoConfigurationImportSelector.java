 ...
 * @version 1.0
 */
public class P6eAuthAutoConfigurationImportSelector implements ImportBeanDefinitionRegistrar {

    /** 默认扫描的包 */
    private static final String DEFAULT_SCAN_PACKAGE = "club.p6e.germ.cloud.auth";
    /** 日志对象 */
    private static final Logger LOGGER = LoggerFactory.getLogger(P6eAuthAutoConfigurationImportSelector.class);

    @Override
    public void registerBeanDefinitions(AnnotationMetadata metadata, BeanDefinitionRegistry registry) {
        try {
            System.out.println(P6eResourceUtil.getResourceContent("auth/index.html"));
        } catch (IOException e) {
            e.printStackTrace();
        }
