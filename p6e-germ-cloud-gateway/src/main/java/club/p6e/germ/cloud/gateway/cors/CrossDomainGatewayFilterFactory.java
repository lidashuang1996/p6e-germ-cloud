 ...
 */
@Component
public class CrossDomainGatewayFilterFactory extends AbstractGatewayFilterFactory<Object> {

    /** 日志对象 */
    private static final Logger LOGGER = LoggerFactory.getLogger(CrossDomainGatewayFilterFactory.class);
    ...
    public static class CustomGatewayFilter implements GatewayFilter, Ordered {
        ...
        public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
            ...
            LOGGER.debug("Cross Domain ==> " + request.getPath());
            final List<String> content = request.getHeaders().get(CROSS_DOMAIN_HEADER_NAME);
            if (content != null && content.contains(CROSS_DOMAIN_HEADER_CONTENT)) {
                // 存在跨域的处理标记，无需处理
                ...
                return chain.filter(exchange.mutate().request(request.mutate().header(
                        CROSS_DOMAIN_HEADER_NAME, CROSS_DOMAIN_HEADER_CONTENT).build()).response(response).build());
            }
        }
