 ...
 * @version 1.0
 */
public final class HuYaProgram extends Program {
    ...
    public synchronized static Application create(String rid) {
        ...
        if (CACHE.get(rid) == null) {
            final Application application = new Application(rid, new LiveRoomCallback.HuYa() {
                ...
                public void onMessage(Client client, List<Message> messages) {
                    execute(HUYA_TYPE, rid, messages);
                }
            });
            // 写入缓存
            put(rid, application);
