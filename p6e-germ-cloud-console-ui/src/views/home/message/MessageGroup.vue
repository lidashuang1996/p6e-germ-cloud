<template>
  <layout-container title="消息中心 / 组管理">
...
export default class MessageGroup extends Vue {
  ...
  private table: TableView<HttpMessageGroupListParam, HttpMessageGroupListItemDataResult> = {
    ...
    headers: [ // 列表头部
      ...
      { title: '限流配置', dataIndex: 'limit', key: 'limit', width: 180 },
      { title: '路由配置', dataIndex: 'route', key: 'route', width: 180 },
      ...
      { title: '操作', key: 'operation', fixed: 'right', width: 82, slots: { customRender: 'operation' } }
    ],
    items: [], // 数据内容
    locale: { // 本地化的语言类型
      filterConfirm: '确定',
  ...
  private async getTableData (): Promise<void> {
    ...
    const res = await Api.message.groupList(param);
