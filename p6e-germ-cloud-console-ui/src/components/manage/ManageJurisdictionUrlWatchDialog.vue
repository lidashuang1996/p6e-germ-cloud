...
/** 数据结构 */
export interface ManageJurisdictionUrlWatchDialogData {
  baseUrl: string;
  config: string;
  createDate: string;
  describe: string;
...
export interface ManageJurisdictionUrlWatchDialogVue extends Vue {
  /** 打开 */
  open(data: ManageJurisdictionUrlWatchDialogData): void;
  /** 关闭 */
  close(): void;
}

@Options({})
export default class ManageJurisdictionUrlWatchDialog
  extends mixins(Mixins.dictionary) implements ManageJurisdictionUrlWatchDialogVue {
  private dialog: {
    view: boolean;
    loading: boolean;
    error: string;
  ...
  /** 重写打开的方法 */
  public open(data: ManageJurisdictionUrlWatchDialogData): void {
    this.reset();
