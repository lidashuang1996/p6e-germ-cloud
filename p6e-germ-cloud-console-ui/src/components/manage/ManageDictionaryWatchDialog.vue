<template>
  <a-modal :footer="null"
    ...
    <div class="message-dictionary-watch-dialog">
      <a-row>
        <a-col :span="24">
        ...
        <a-col :span="12">
          <span class="title">键</span>
          <span class="content">{{ dialog.key }}</span>
        </a-col>
        <a-col :span="12">
          <span class="title">值</span>
          <span class="content">{{ dialog.value }}</span>
        </a-col>
        <a-col :span="12">
          <span class="title">语言</span>
          <span class="content">{{ dialog.language }}</span>
        </a-col>
        <a-col :span="12">
...
/** 数据结构 */
export interface ManageDictionaryWatchDialogData {
  key: string;
  id: number;
  type: string;
  language: string;
...
export interface ManageDictionaryWatchDialogVue extends Vue {
  /** 打开 */
  open (data: ManageDictionaryWatchDialogData): void;
  /** 关闭 */
  close (): void;
}

@Options({})
export default class ManageDictionaryWatchDialog extends Vue implements ManageDictionaryWatchDialogVue {
  private dialog: {
    view: boolean;
    loading: boolean;
    error: string;
  ...
  /** 重写打开的方法 */
  public open (data: ManageDictionaryWatchDialogData): void {
    this.reset();
...
.message-dictionary-watch-dialog {
  font-size: 14px;
  line-height: 28px;
  .title {
